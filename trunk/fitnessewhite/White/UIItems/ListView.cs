...
namespace Core.UIItems
{
    ...
    /// </summary>
    public class ListView : UIItem, SuggestionList
    {
        ...
        {
            int i = Header.Columns[column].Index;
            return Rows.Get(zeroBasedRowIndex).Cells[i];
        }
        ...
        {
            SafeAutomationEventHandler safeAutomationEventHandler =
                new SafeAutomationEventHandler(this, eventListener,
                                               delegate(object[] objs) { return ListViewEvent.Create(this, (AutomationPropertyChangedEventArgs) objs[0]); });
        ...
        {
            ListViewRows rows = Rows;
            ListViewRow row = rows.Find(delegate(ListViewRow obj) { return obj.Cells[0].Text.Equals(text); });
