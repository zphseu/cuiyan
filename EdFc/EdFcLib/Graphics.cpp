...
CGraphics::CGraphics()
{
    ...
    crTable[8]  = RGB(255, 255,   0);     // Yellow
	...
	crTable[15] = RGB(  0,   0,   0);     // Black

	crTable[16]  = RGB(100, 0, 0);     
    crTable[17]  = RGB(0, 100, 0);     
	crTable[18]  = RGB(0,  10,  100);     
	crTable[19]  = RGB( 50,0,   0);     
	crTable[20]  = RGB(  0,   50, 50);     
	crTable[21]  = RGB(0,   0, 50);     
	crTable[22]  = RGB(  80, 0,0);     
	crTable[23]  = RGB(0, 80, 0);     
	crTable[24]  = RGB(0, 0,   80);
	crTable[25]  = RGB(80,  80, 0);     
	crTable[26] = RGB(  0, 80, 80);     
	crTable[27] = RGB(80,   0,   80);     
	crTable[28] = RGB(  150, 150,   0);     
	crTable[29] = RGB(  0,   150, 150);     // 
	crTable[30] = RGB(150, 0, 135);     // 
	crTable[31] = RGB(  150,  150,150);     
...
void CGraphics::RecalcRects(RECT& rt)
{
	...
	m_lM  = 2 * (m_Rect.right - m_Rect.left) / 21;
	m_rM  = (m_Rect.right - m_Rect.left) / 8;
...
{
	m_LogFont.lfHeight = (LONG)(m_Size.cx / -15.0);
...
void CGraphics::Format(int decimal, char* str, double value)
{
	char tstr[24];
	switch(decimal)
	{
		...
		for(i = 1; i <= m_nYStep; i ++)
		{
			for(j = 1; j <= 10; j ++)
			{
				if (k == 0)   
					...
					xe = GR + m_rM / 20;
				yb = ye = GB - (int)(log10(j * 1.0) * PY / m_nYStep + 1.0 * (i - 1) * PY / m_nYStep);
				if (j == 1)
				{
