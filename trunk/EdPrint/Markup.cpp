...
bool CMarkup::SetDoc( LPCTSTR szDoc )
{
	...
	const int nBaseSize = 50;
	if ( m_aPos.GetSize() < nBaseSize )
		m_aPos.SetSize( nBaseSize );
...
CString CMarkup::DebugMap() const
{
	// Return a dump of the m_aPos array for debugging purposes
	//
	// Prepare to test chars in the document
	LPCTSTR pszDoc = (LPCTSTR)m_csDoc;
	int nLenDoc = m_csDoc.GetLength();
	#define DOC_CHAR(p) (nLenDoc>p)?pszDoc[p]:'!'

	// Loop through positions in document
	CString csLine, csDump;
	for ( int iPos = 1; iPos < m_iPosFree; ++iPos )
	{
		// Check chars pointed to by indexes
		// Empty szChars if correct
		_TCHAR szChars[6];
		memset(szChars,0,sizeof(szChars));
		szChars[0] = DOC_CHAR(m_aPos[iPos].nStartL);
		szChars[1] = DOC_CHAR(m_aPos[iPos].nStartR);
		szChars[2] = DOC_CHAR(m_aPos[iPos].nEndL);
		szChars[3] = DOC_CHAR(m_aPos[iPos].nEndR);
		szChars[4] = DOC_CHAR(m_aPos[iPos].nNext);
		szChars[5] = '\0';
		if ( szChars[4] == '<' || szChars[4] == '!' )
		{
			if ( m_aPos[iPos].IsEmptyElement() )
			{
				if ( _tcsncmp(_T("<>/>"),szChars,4) == 0 )
					szChars[0] = '\0';
			}
			else
			{
		...
		// Build line of dump
		bool bSafe = (m_aPos[iPos].nStartL >= 0 && m_aPos[iPos].nNext <= nLenDoc);
		csLine.Format( _T("Index %d %.32s\t%d %d %d %d %d [%s] p=%d n=%d c=%d\n"),
			iPos,
			bSafe? x_GetTagName(iPos) : "UNSAFE",
			m_aPos[iPos].nStartL,
			m_aPos[iPos].nStartR,
			m_aPos[iPos].nEndL,
			m_aPos[iPos].nEndR,
			m_aPos[iPos].nNext,
			bSafe?(szChars[0]?szChars:_T("valid")):_T(""),
	...
	while ( 1 )
	{
		...
		while ( x_FindToken( token ) )
		{
			...
			if ( ! token.bIsString )
			{
				...
				_TCHAR cEndCheck = m_csDoc[token.nL];
				if ( cEndCheck == '/' )
				{
					if ( nTokenCount == 1 )
						return x_ReleasePos(); // end mark at beginning
					if ( nTagFlag == 0 )
				...
				{
					if ( m_csDoc[token.nR] == '-' && token.nR == token.nL + 1 )
		...
		// Ignore tag
		m_aPos[iPos].nStartL = token.nL;
		m_aPos[iPos].nNext = token.nL + 1;
	}

	// Was a right angle bracket found?
	if ( ! token.IsValid() )
		return x_ReleasePos();
	m_aPos[iPos].nStartR = token.nL;
		...
		while ( x_FindToken( token ) )
		{
			...
			if ( ! token.bIsString )
			{
				...
				if ( nTokenCount == 1 && m_csDoc[token.nL] != '/' )
					return x_ReleasePos(); // expected end mark

				else if ( nTokenCount == 2 && csName != x_GetToken( token ) )
					return x_ReleasePos(); // name does not correspond
...
bool CMarkup::x_SetData( int iPos, LPCTSTR szData )
{
	// Set data in iPos element
	ElemPos& ep = m_aPos[iPos];
	if ( ! iPos || ep.iElemChild )
		return false;
	CString csInsert = x_TextToDoc( szData );
...
void CMarkup::x_DocChange( int nLeft, int nReplace, const CString& csInsert )
{
	...
	_TCHAR* pDoc = m_csDoc.GetBuffer( nNewLength );
