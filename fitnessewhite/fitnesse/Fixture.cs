...
namespace fit
{
	public class Fixture
	{
		...
		public virtual void DoTables(Parse tables)
		{
            ...
            try {
                Fixture fixture = LoadFixture(tables.At(0, 0, 0).Text);
                fixture.ProcessTables(tables, this);
            }
            catch (Exception e) {
                Exception(tables.Parts.Parts, e);
                Listener.TableFinished(tables);
            }
			Listener.TablesFinished(Counts);
		}

        public virtual void ProcessTables(Parse theTables, Fixture theParentFixture) {
            Fixture fixture = this;
            for (Parse table = theTables; table != null; table = table.More) {
                Parse heading = table.At(0, 0, 0);
                if (heading != null) {
                    try {
                        if (fixture == null) fixture = LoadFixture(heading.Text);
                        fixture.Prepare(theParentFixture, table);
                ...
                theParentFixture.Listener.TableFinished(table);
                fixture = null;
            }
        }

	    public void Prepare(Fixture theParentFixture, Parse table) {
	        Counts = theParentFixture.Counts;
	        Summary = theParentFixture.Summary;
