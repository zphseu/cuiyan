...
void CGradient::DrawLinearGradient(CDC *pDC, CRect rcGrad, int nClipStart, int nClipEnd, int nShift)
{
	...
	int nSteps = (int)arElements.GetSize();
                                  ...
                                  int nClipStart, int nClipEnd, UINT nMaxColors)
{
	...
	int nCount = (int)arElementsOrig.GetSize();
	if(nCount < 2)
	{
		// no gradient
	...
	// do clip
	if(nClipEnd == -1) nClipEnd = nGradWidth;
	nCount = (int)arElementsShift.GetSize();
	nLength = 0;
	for(i = 0; i < nCount; i++)
	{
...
void CGradient::CreatePalette()
{
	...
	nNumColors = (int)arElements.GetSize();
	if(!nNumColors)
		return;
...
{
	int nCount = (int)m_arflGradStretch.GetSize();
...
{
	int nCount = (int)m_ardwGradColors.GetSize()-1;
	if(nCount < 1)
	{
