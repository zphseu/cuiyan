							  ...
							  LPCTSTR lpszData)
{
    if (m_arFolder.GetSize() != 0)
    {
        ...
        CXTBarFolder* pBarFolder = (CXTBarFolder*)m_arFolder.GetAt(iFolder);
        return pBarFolder->InsertItem(iIndex, iImage, lpszItemName, lpszData);
    }
    return -1;
}
...
CXTBarItem::CXTBarItem(const int nImageIndex, LPCTSTR lpszName, LPCTSTR lpszData) :
    m_nIndex(nImageIndex), m_strName(lpszName), m_strData(lpszData)
{
}
...
int CXTBarFolder::InsertItem(int iIndex, const int nImage, LPCTSTR lpszName, LPCTSTR lpszData)
{
    ...
    CXTBarItem* pBarItem = new CXTBarItem( nImage, lpszName, lpszData);
    ASSERT(pBarItem);
