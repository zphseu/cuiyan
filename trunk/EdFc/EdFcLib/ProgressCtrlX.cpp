...
{
	CProgress::SetBkColor((COLORREF)clrBk);
	return CLR_DEFAULT;
}

LRESULT CProgressCtrlX::OnSetStep(WPARAM nStepInc, LPARAM)
{
	int res = CProgress::SetStep((int)nStepInc);
	Invalidate();
	return res;
}
...
{
	int res = CProgress::SetPos((int)newPos);
	Invalidate();
	return res;
}
...
LRESULT CProgressCtrlX::OnSetRange32(WPARAM low, LPARAM hi)
{
	...
	CProgress::SetRange((int)low, (int)hi);
	Invalidate();
...
{
	int res = CProgress::OffsetPos((int)nIncrement);
	Invalidate();
	return res;
}
