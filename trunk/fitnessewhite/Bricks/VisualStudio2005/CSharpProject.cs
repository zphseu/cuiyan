...
namespace Bricks.VisualStudio2005
{
    public class CSharpProject : VisualStudioProject
    {
        ...
        public virtual void DoXmlDocumentation(string buildConfiguration)
        {
            ...
            XmlNode documentationNode = DocumentationNode(propertyGroup);
            documentationNode.InnerText = XML.ChildNode(propertyGroup, "OutputPath").InnerText + GetAssemblyName() +
                                          ".xml";
        }

        private XmlNode DocumentationNode(XmlNode propertyGroup)
        {
            string documentationNodeName = "DocumentationFile";
            XmlNode documentationNode = XML.ChildNode(propertyGroup, documentationNodeName);
            if (documentationNode == null)
            {
                documentationNode =
                    xmlDocument.CreateNode(XmlNodeType.Element, documentationNodeName, propertyGroup.NamespaceURI);
        ...
        {
            List<XmlNode> list =
                XML.MatchingNodes(xmlDocument, "PropertyGroup",
                                  delegate(XmlNode obj) { return obj.Attributes.Count == 0; });
        ...
        public virtual DirectoryInfo OutputDir(string configurationName)
        {
            ...
            return new DirectoryInfo(new FileInfo(location).Directory.FullName + @"\" + projectOutput);
        }
        ...
        {
            List<XmlNode> itemGroups = XML.MatchingNodes(xmlDocument, "ItemGroup", delegate(XmlNode obj)
                                                                              {
                                                                                  return obj.ChildNodes.Count != 0 && obj.ChildNodes[0].Name.Equals("ProjectReference");
                                                                              });
