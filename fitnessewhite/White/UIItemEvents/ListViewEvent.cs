...
namespace Core.UIItemEvents
{
    public class ListViewEvent : UserEvent
    {
        ...
        {
            int columnPosition = Mouse.Instance.Location.X;
            ...
            else if (true.Equals(eventArgs.NewValue))
            {
                string column = null;
                if (listView.Header != null)
                {
                    ListViewColumn listViewColumn =
                        listView.Header.Columns.Find(
                            delegate(ListViewColumn obj) { return obj.Bounds.Left < columnPosition && columnPosition < obj.Bounds.Right; });
