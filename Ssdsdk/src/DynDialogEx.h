...
public:
	...
	virtual CWnd *GetParent() {	return m_pParentWnd; }
...
// Dialog Data
	...
	virtual UINT AddDlgControl(DLGITEMTEMPLATECONTROLS TypeControl,
			LPCTSTR lpszCaption, DWORD dwStyle, DWORD dwExtendedStyle,
			LPRECT pRect = NULL, VOID *pData = NULL, UINT nID = 0);
	...
	virtual UINT AddSubclassedDlgControl(LPCSTR lpszClassName,
			LPCTSTR lpszCaption, DWORD dwStyle, DWORD dwExtendedStyle,
			LPRECT pRect = NULL, UINT nID = 0);
	
	void SetFont(CFont *pFont)	{ m_pFont = pFont; }
	CFont* GetFont()			{ return m_pFont; }
	void SetFontSize(WORD wSize){ m_wFontSize = wSize; }
	WORD GetFontSize()			{ return m_wFontSize; }
	void SetUseSystemButtons(BOOL bUse = TRUE) { m_bAddSystemButtons = bUse; }

	void SetWindowTitle(LPCSTR lpszCaption) { m_strCaption = lpszCaption; }
	void SetUseModeless(BOOL bModelessDlg = TRUE) { m_bModelessDlg = bModelessDlg; }
	long GetNumberOfConrols() {return m_arrDlgItemPtr.GetSize();}

	//Additional functions by Tom Daffin
	void AddStyles(DWORD dwStyles) { m_DialogTemplate.style |= dwStyles; }
	void RemoveStyles(DWORD dwStyles) { m_DialogTemplate.style &= (~dwStyles); }
